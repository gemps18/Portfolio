  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-line bg-ink/85 backdrop-blur">
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        <ul className="hidden items-center gap-8 font-body text-sm font-medium text-slate md:flex">
          {NAV_ITEMS.map((item) => (
            <li key={item.href}>
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="#contact"
            className="hidden rounded border border-signal px-4 py-2 font-mono text-xs text-signal transition-colors hover:bg-signal hover:text-ink sm:inline-block"
          >
            {t.nav.cta}
          </a>
        </div>
